...
app.config['MYSQL_PASSWORD'] = 'Patil@2002'
app.config['MYSQL_DB'] = 'quizapp'
app.config['MYSQL_CURSORCLASS'] = 'DictCursor'

app.config['MAIL_SERVER']='smtp.gmail.com'
app.config['MAIL_PORT'] = 587
app.config['MAIL_USERNAME'] = 'adityabp192002@gmail.com'
app.config['MAIL_PASSWORD'] = 'fewvaurzvpvajbyv'
...
mysql = MySQL(app)
sender = 'adityabp192002@gmail.com'
YOUR_DOMAIN = 'http://localhost:5000'
MOVEMENT_THRESHOLD_TIME = 20
...
def get_authentication_image():
    if 'authentication_image' not in session:
        # if not authentication_image
        email = session['email']
        user_type = session['user_role']
        cur = mysql.connection.cursor()
        get_authentication_image = cur.execute(
            'SELECT user_image from users where email = %s and user_type = %s', (email, user_type)
        )
        cresults = cur.fetchone()
        imgdata2 = cresults['user_image']
        session['authentication_image'] = imgdata2
        cur.close()
        
        return session['authentication_image']	
    else:
        return session['authentication_image']


@app.route('/video_feed', methods=['GET', 'POST'])
@user_role_student  # Ensure this decorator is defined elsewhere
def video_feed():
    if request.method == "POST":
        # Initialize the default status and message
        status = "success"
        message = "Recorded image of video"
        ...
        if voice_db >= 20.0:
            message = "There is disturbance"
            status = "cheating"
        
        # Get authentication image for verification
        authentication_image = get_authentication_image()
        
        # Process the image data
        proctorData = camera.get_frame(imgData, authentication_image)
        
        # Extract data from the processed results
        ...
        authentication_status = proctorData['verification_status']
        head_movements = proctorData['head_movements']['head_position']
        
        # Determine head movement binary status
        head_movements_binery = 0 if head_movements != 'Forward' or head_movements == 'No face detected' else 1

        # Check for cheating indicators
        if person_status != 0:
            message = "More than one person detected"
            status = "cheating"
            session['number_of_alerts'] = session.get('number_of_alerts', 0) + 1
        ...
        # Initialize movement data in session if not present
        if 'movement_data' not in session:
            session['movement_data'] = {
                'user_move1': {'start_time': None},
                'user_move2': {'start_time': None},
                'eye_movements': {'start_time': None},
                'head_movements': {'start_time': None}
            }
        if 'number_of_alerts' not in session:
            session['number_of_alerts'] = 0     

        movement_data = session['movement_data']
        
        # Check head movement directions
        user_move1 = 1 if head_movements in ['Looking Left', 'Looking Right'] else 0
        user_move2 = 1 if head_movements in ['Looking Down', 'Looking Up'] else 0

        # Helper function to track movements
        def track_movement(movement_name, movement_value):
            current_time = time.time()
            movement_info = movement_data[movement_name]

            if movement_value != 0:
                # Start tracking if movement is detected
                if movement_info['start_time'] is None:
                    movement_info['start_time'] = current_time
                elif current_time - movement_info['start_time'] > MOVEMENT_THRESHOLD_TIME:
                    print("current time", current_time)
                    # If movement has lasted more than 30 seconds, set cheating status
                    return "cheating"
            else:
                # Reset timer if no movement
                movement_info['start_time'] = None

            return "success"

        # Track movements and set status
        status_eye_movements = track_movement('eye_movements', eye_movements)
        status_head_movements = track_movement('head_movements', head_movements_binery)

        # Update session with the new movement data
        session['movement_data'] = movement_data

        if status_eye_movements == "cheating":
            status = "cheating"
            message = "Suspicious activity detected head or eyes are not at correct position."

        # If any movement indicates cheating, update overall status
        if status_head_movements == "cheating":
            status = "cheating"
            message = "Suspicious activity detected head or eyes are not at correct position."
            session['number_of_alerts'] = session['number_of_alerts'] + 1
        
        if session['number_of_alerts'] >= 3:
            status = "autosubmit"
        
        cur = mysql.connection.cursor()
        result = cur.execute(
            'INSERT INTO proctoring_log (email, name, test_id, voice_db, img_log, user_movements_updown, user_movements_lr, user_movements_eyes, phone_detection, person_status, uid, head_movement) '
            'VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)',
            (dict(session).get('email'), dict(session).get('name'), testid, voice_db, jpg_as_text, user_move1, user_move2, eye_movements, mob_status, person_status, dict(session).get('uid'), head_movements)
        )
        ...
        if status:
            ...
            return jsonify({"message": "Error recording video", "status": "error"}), 500
...
def report_professor_email():
	if request.method == 'POST':
		careEmail = "adityabp192002@gmail.com"
		cname = session['name']
...
def lostpassword():
	if request.method == 'POST':
		...
		if results > 0:
			...
			msg1 = Message('ExamProctor - OTP Verification for Lost Password', sender = sender, recipients = [lpemail])
...
def lpnewpwd():
	if request.method == 'POST':
		...
		if(npwd == cpwd ):
			...
			cur.close()
			session.clear()
			return render_template(url_for('login'),success="Your password was successfully changed.")
		else:
...
def register():
	if request.method == 'POST':
		...
		msg1 = Message('ExamProctor - OTP Verification', sender = sender, recipients = [email])
...
def register():
    if request.method == 'POST':
        ...
        # Inserting user data directly into the database
        try:
            cur = mysql.connection.cursor()
            cur.execute(
...
def login():
    if request.method == 'POST':
        ...
        user_type = request.form['user_type']
        imgdata1 = request.form.get('image_hidden')  # Ensure this is fetched correctly
        ...
        if results1 > 0:
            ...
            name = cresults['name']
            uid = cresults['uid']
            user_login_status = cresults['user_login']
            
            # Check if the user is already logged in
            if user_login_status == 1:
                # Optionally, you could log the user out before continuing
                # Uncomment the lines below if you want to force logout before a new login
                cur.execute('UPDATE users set user_login = 0 where email = %s', [email])
                mysql.connection.commit()

                # Or simply allow the login to proceed by commenting out the return statement below
                #error = 'User is already logged in. Please log out first.'
                #return render_template('login.html', error=error)

            # Proceed with image verification and password check
            ...
            if img_result["verified"] == True and password == password_candidate:
                ...
                if results2 > 0:
                    ...
                    print("this is session before loging off",session)
                    if user_type == "student":
                        return render_template('student_index.html')
                    else:
                        return render_template('professor_index.html')
                else:
                    error = 'Error updating login status in the database.'
                    return render_template('login.html', error=error)
            else:
                error = 'Image verification failed or incorrect password.'
                return render_template('login.html', error=error)
...
def logout():
    try:
        # Ensure that the required session data exists
        email = session.get('email')
        uid = session.get('uid')
        
        if not email or not uid:
            raise Exception("Session data is missing. Cannot log out properly.")
...
#         if not email or not uid:
#             return jsonify({"status": "error", "message": "Session data is missing. Cannot log out properly."}), 400
        
#         # Update user login status in the database
#         cur = mysql.connection.cursor()
#         cur.execute('UPDATE users SET user_login = 0 WHERE email = %s AND uid = %s', (email, uid))
#         mysql.connection.commit()
#         cur.close()
...
def create_test():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        # Ensure DataFrame contains only the expected fields
        df = pd.DataFrame(ef, columns=fields)
        
        # Clean DataFrame to replace NaNs and convert data types
        df = df.fillna({
            'qid': 0,    # Assuming qid should not be NaN
            'q': '',     # Assuming q should not be NaN
            'a': '',     # Assuming a should not be NaN
            'b': '',     # Assuming b should not be NaN
            'c': '',     # Assuming c should not be NaN
            'd': '',     # Assuming d should not be NaN
            'ans': '',   # Assuming ans should not be NaN
            'marks': 1   # Set default value for marks
        })
        
        # Convert 'marks' to integer type
            ...
            try:
                ...
                for row in df.index:
                    cur.execute(
                        ...
                        (test_id, df['qid'][row], df['q'][row], df['a'][row], df['b'][row], df['c'][row], df['d'][row], df['ans'][row], df['marks'][row], session['uid'])
                    )
                ...
                cur.execute(
                    ...
                    (session['email'], test_id, "objective", start_date_time, end_date_time, duration, 1, password, subject, topic, neg_mark, calc, proctor_type, session['uid'])
                )
                mysql.connection.commit()
...
def displaystudentsdetails():
    if request.method == 'POST':
        ...
        print(f"Selected Test ID: {tidoption}")  # Debugging
        if tidoption:
            cur = mysql.connection.cursor()
            ...
            if callresults:
                ...
                flash('No student details found for the selected Test ID.', 'info')
        else:
            flash('Please select a Test ID.', 'warning')
    return render_template("displaystudentsdetails.html", callresults=[])
...
def give_test():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if results1 > 0:
            ...
            # Decode base64 images into numpy arrays
            try:
                nparr1 = np.frombuffer(base64.b64decode(imgdata1), np.uint8)
                nparr2 = np.frombuffer(base64.b64decode(imgdata2), np.uint8)
                ...
                # Check for proper image decoding
                if image1 is None or image2 is None or image1.ndim != 3 or image2.ndim != 3:
                    flash('Error decoding images, please try again.', 'danger')
                ...
                if img_result["verified"]:
                    ...
                    if results > 0:
                        ...
                        # Validate password and exam timing
                        if password == password_candidate:
                            ...
                            if datetime.strptime(start, "%Y-%m-%d %H:%M:%S") < now and datetime.strptime(end, "%Y-%m-%d %H:%M:%S") > now:
                                ...
                                if results > 0:
                                    ...
                                    # Insert initial test information if not present
                ...
                flash(f'Error in image verification: {str(e)}', 'danger')
                return redirect(url_for('give_test'))
...
def test(testid):
    ...
    if callresults['test_type'] == "objective":
        ...
        if request.method == 'GET':
            ...
            cur.execute('SELECT COUNT(qid) as total_questions from longqa where test_id = %s', [testid])
            results1 = cur.fetchone()
            total_questions = results1['total_questions']
            cur.close()

            insertStudentData = None
            for sa in range(1, total_questions + 1):
                answerByStudent = request.form[str(sa)]
                cur = mysql.connection.cursor()
                cur.execute('SELECT marks from longqa where test_id = %s and qid = %s', (testid, sa))
                questionData = cur.fetchone()
                marks = questionData['marks'] if questionData else 0

                insertStudentData = cur.execute(
                    'INSERT INTO longtest(email, test_id, qid, ans, uid, marks) values(%s, %s, %s, %s, %s, %s)',
                    (session['email'], testid, sa, answerByStudent, session['uid'], marks)
                )
                mysql.connection.commit()
                cur.close()
        ...
        if request.method == 'GET':
            ...
            insertStudentData = cur.execute(
                ...
                (session['email'], testid, "1", codeByStudent, inputByStudent, executedByStudent, session['uid'])
            )
            mysql.connection.commit()
            if insertStudentData > 0:
                insertStudentTestInfoData = cur.execute(
...
def check_result(email, testid):
    if email == session['email']:
        ...
        if results > 0:
            ...
            show_answers = test_info['show_ans']
            if show_answers == 1:
                query = """
                SELECT q.q, q.a, q.b, q.c, q.d, q.marks, q.qid, 
                q.ans AS correct, IFNULL(s.ans, '0') AS marked 
                FROM questions q 
                LEFT JOIN students s 
                ON s.test_id = q.test_id 
                AND s.test_id = %s 
                AND s.email = %s 
                AND s.uid = %s 
                AND s.qid = q.qid 
                ...
                """
                cur.execute(query, (testid, email, session['uid'], testid))
                results = cur.fetchall()
...
def neg_marks(email, testid, negm):
    ...
    ORDER BY q.qid ASC
    """
    cur.execute(query, (testid, email, testid))
    data = cur.fetchall()

    total_score = 0.0
    for row in data:
        marked = str(row['marked']).upper()
        correct = str(row['correct']).upper()
        marks = int(row['marks'])

        if marked != '0':  # If a question was marked
            if marked != correct:
                # Apply negative marking for incorrect answers
                total_score -= (negm / 100) * marks
            else:
...
def totmarks(email, tests):
    ...
    for test in tests:
        ...
        if results > 0:
            result = cur.fetchone()
            negm = result['neg_marks']
            total += neg_marks(email, testid, negm)
    return total
