...
# Load class names (e.g., COCO dataset)
...
def track_head_movement(image):
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            # Get the rotational matrix and the angles
            rmat, _ = cv2.Rodrigues(rot_vec)
            angles, _, _, _, _, _ = cv2.RQDecomp3x3(rmat)

            # Get rotation around x, y, and z axes (in degrees)
            x_angle = angles[0] * 360
            y_angle = angles[1] * 360
            z_angle = angles[2] * 360

            # Determine the head movement based on y_angle and x_angle
            if y_angle < -10:
            ...
            # Optionally, you can draw the line on the image to show nose direction (if needed)
            return {
    ...
    return {'head_position': 'No face detected', 'x_angle': 0, 'y_angle': 0, 'z_angle': 0}    
    

def get_frame(imgData,authentication_image):
    nparr = np.frombuffer(base64.b64decode(imgData), np.uint8)
    image = cv2.imdecode(nparr, cv2.COLOR_BGR2GRAY)
    
    frame = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    
    authentication_image_decode = np.frombuffer(base64.b64decode(authentication_image), np.uint8)
    authentication_image = cv2.imdecode(authentication_image_decode, cv2.COLOR_BGR2GRAY)
    
    verification_status  = DeepFace.verify(image, authentication_image, enforce_detection=False)
    
    head_position = track_head_movement(frame)
    ...
    #         # Check if the detection is a cell phone with a confidence > 0.6
    #         if confidence > 0.6 and class_id == cell_phone_class_id:
    #             mob_status= 1  # Cell phone detected
    #         else:
    #             mob_status = 0  # Not a cell phone
    ...
    proctorDict['verification_status'] = verification_status
    proctorDict['head_movements']=head_position
