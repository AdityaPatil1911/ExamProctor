...
window.onfocus = function(event) {
  ...
  function captureSnapshot() {
      ...
      var res = d1.replace("data:image/png;base64,", "");
        ...
        if(average)
        {
            $.post("/video_feed", {
                ...
                if (response.status === "cheating") {
                    // Handle the cheating status (e.g., call alert function)
                    showSuspiciousAlert(response.message);
                } else if(response.status=='autosubmit'){
                    finishTest()
                }
                else
                {
    ...
    function showSuspiciousAlert(message) {
        const alertBox = document.getElementById('suspicious-alert');
        const alertMessage = document.getElementById('alert-message');
        
        alertMessage.innerText = message; // Set the alert message
        alertBox.style.display = 'flex';  // Show alert box
        setTimeout(() => {
          alertBox.style.display = 'none';  // Hide alert box after 5 seconds
        }, 3000);
      }

      function finishTest() {
            showSuspiciousAlert("Suspicious activity detected Auto submitting the test")// Alert message
            // Wait for 3 seconds (3000 milliseconds) before submitting the form
            setTimeout(function() {
                // Simulate form submission
                document.getElementById('finish').submit(); // Submit the form directly
